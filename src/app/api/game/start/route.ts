...
export async function POST() {
  try {
    ...
    // 2. Select 5 diverse, strictly non-repeating landmarks across campus
    const selectedLocations = selectDistinctGameLocations(allLocations, 5);

    if (selectedLocations.length < 5) {
      return NextResponse.json(
        { error: 'Could not select 5 unique location images.' },
        { status: 500 }
      );
    }
