    ...
    >
      ...
      <div className={`absolute top-2 ${isMobileMode ? 'right-2' : 'right-12'} z-[500] flex items-center gap-1`}>
        ...
        <button
          ...
          aria-label="Reset to Campus Center"
          className="p-2 rounded-full bg-slate-900/80 hover:bg-slate-800 text-slate-300 border border-slate-700/60 transition min-w-[32px] min-h-[32px] flex items-center justify-center"
        >
      ...
      <div className="p-2.5 sm:p-3 pb-[calc(env(safe-area-inset-bottom,0px)+0.75rem)] bg-slate-900/95 border-t border-slate-800 flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5">
          {guessCoord ? (
            <button
              ...
              aria-label="Clear selected guess pin"
              className="px-2.5 py-2 rounded-lg bg-slate-800 hover:bg-slate-750 text-slate-400 hover:text-red-400 text-xs font-semibold flex items-center gap-1 transition min-h-[36px]"
            >
            ...
            <button
              ...
              disabled={disabled || isSubmitting}
              aria-label="Pin guess at campus center"
              className="px-3 py-2 rounded-lg bg-slate-800 hover:bg-slate-750 text-amber-300 text-xs font-bold border border-slate-700 flex items-center gap-1 transition active:scale-95 min-h-[36px]"
            >
        ...
        <button
          ...
          aria-label={guessCoord ? 'Lock in your campus guess' : 'Place a pin on the map to lock guess'}
          className={`px-4 sm:px-5 py-2 rounded-xl font-bold text-xs sm:text-sm tracking-wider uppercase transition-all shadow-md flex items-center gap-2 min-h-[40px] ${
